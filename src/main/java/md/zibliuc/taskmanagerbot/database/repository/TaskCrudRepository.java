...
public interface TaskCrudRepository extends CrudRepository<Task, Long> {
    ...
    @Query("SELECT t FROM Task t WHERE t.deadline >= :start AND t.deadline <= :end AND t.isCompleted = :isCompleted AND t.sendNotification = :sendNotification")
    List<Task> findByDeadlineBetweenAndIsCompletedAndSendNotification(
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end,
            @Param("isCompleted") boolean isCompleted,
            @Param("sendNotification") boolean sendNotification
    );
}
